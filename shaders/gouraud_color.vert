...
void main() {
    ...
    vec3 lightDirection = normalize(light_position - vec3(vertex_world_position));
    ...
    float hold = clamp(dot(normalizedVNormal,lightDirection),0.0, 1.0);
    
    //-this will hold it to 0.0 if it is negative
    float diffuseIntensity = max(hold, 0.0);
